...
Here we link the wandb [experiments' learning curves](https://wandb.ai/deep-learning-project/dl-project) and the [Google Colab notebook link TODO]().
...
Having only few examples available, training a network from scratch is will not give satisfactory results; that is the reason why we have decided to always start with a feature extractor pre-trained on the [ImageNet](https://ieeexplore.ieee.org/document/5206848) object recognition dataset.

Moreover, in order to delve into the proposed methods, we evaluate the domain adaptation method starting from two different backbones networks, namely [AlexNet](https://proceedings.neurips.cc/paper/2012/file/c399862d3b9d6b76c8436e924a68c45b-Paper.pdf) and [ResNet18](https://arxiv.org/abs/1512.03385?context=cs).
